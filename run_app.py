...
def kill_port(port):
    ...
    try:
        ...
        if port == 80:
            osa_cmd = ['osascript', '-e', f'do shell script "lsof -ti:{port} | xargs kill -9" with administrator privileges']
            subprocess.run(osa_cmd, stderr=subprocess.DEVNULL)
        else:
...
def main():
    ...
    print("✅ 자막 전용 웹 서버(server.py)를 구동합니다... (비밀번호 입력 창이 뜰 수 있습니다)")
    server_cmd = f"cd {base_dir}/LanguageLearner && {sys.executable} server.py < /dev/null > /tmp/server.log 2>&1 &"
    osa_cmd = ['osascript', '-e', f'do shell script "{server_cmd}" with administrator privileges']
    server_process = subprocess.Popen(osa_cmd)
    ...
    # 4. 사용자의 웹 브라우저 띄우기
    print("🌐 웹 브라우저를 엽니다...")
    webbrowser.open("http://ggeolmu-language.com")
